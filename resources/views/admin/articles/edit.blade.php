...
<head>
    ...
    <title>Edit Artikel — Admin KontenDigital</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-view { font-family: 'Space Grotesk', sans-serif; font-weight: 900; font-size: 0.75rem; letter-spacing: 0.08em; color: var(--purple); text-decoration: none; border: 3px solid var(--ink); padding: 0.5rem 1rem; background: var(--yellow); display: inline-flex; align-items: center; gap: 0.5rem; transition: 0.15s; text-transform: uppercase; }
        .btn-view:hover { transform: translate(1px, 1px); }
        .current-thumb { border: 3px solid var(--ink); max-height: 150px; margin-bottom: 0.75rem; display: block; object-fit: cover; }
        ...
        @media (max-width: 600px) {
            ...
            .current-thumb { width: 100%; max-height: 200px; }
        }
    </style>
</head>
<body>
...
<div class="sidebar-logo-box">
    ...
    <div class="main">
        <div class="topbar">
            <div style="display: flex; align-items: center;">
                ...
                <div>
                    ...
                    <div class="topbar-breadcrumb">Admin / Artikel / Edit: {{ Str::limit($article->title, 30) }}</div>
                </div>
            </div>
            <div class="topbar-actions" style="display:flex; gap:0.75rem; align-items:center;">
                <a href="{{ route('articles.show', $article->slug) }}" target="_blank" class="btn-view">
        ...
        <div class="content">
            @if(session('success'))
                <div style="background:rgba(0,168,150,0.12); border:2px solid var(--teal); padding:0.75rem 1.25rem; font-weight:700; font-size:0.85rem; color:#004d47; margin-bottom:1.5rem;">
                    ✓ {{ session('success') }}
                </div>
            @endif
            ...
            <div class="form-card">
                <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="field-group">
                        ...
                        <input id="title" name="title" type="text" class="field" value="{{ old('title', $article->title) }}" required maxlength="255" oninput="updateCharCount(this,'title-count',255)">
                        <div style="font-size:0.7rem; text-align:right; margin-top:5px; color:rgba(14,11,20,0.4);"><span id="title-count">0</span>/255</div>
                    ...
                    <div class="field-group">
                        ...
                        @if($article->thumbnail)
                            <img src="{{ asset('storage/'.$article->thumbnail) }}" class="current-thumb" alt="current thumbnail">
                            <div class="field-hint" style="margin-bottom:0.5rem;">Upload baru untuk mengganti.</div>
                        @endif
                        <input type="file" name="thumbnail" class="field" accept="image/*" style="padding:0.4rem;">
                    ...
                    <div class="field-group">
                        ...
                        <textarea id="excerpt" name="excerpt" class="field" style="min-height:80px;" maxlength="300" oninput="updateCharCount(this,'excerpt-count',300)">{{ old('excerpt', $article->excerpt) }}</textarea>
                        <div style="font-size:0.7rem; text-align:right; margin-top:5px; color:rgba(14,11,20,0.4);"><span id="excerpt-count">0</span>/300</div>
                    </div>

                    <div class="field-group">
                        <label for="content">Isi Artikel <span>*</span></label>
                        <textarea id="content" name="content" class="field" required oninput="updateCharCount(this,'content-count',null)">{{ old('content', $article->content) }}</textarea>
                        <div style="font-size:0.7rem; text-align:right; margin-top:5px; color:rgba(14,11,20,0.4);"><span id="content-count">0</span> karakter</div>
                    </div>
